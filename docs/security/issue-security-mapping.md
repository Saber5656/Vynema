...
| #9 Development media capability and `StorageAdapter` policy | Media privacy and scoped writes | Storage policy evidence that pending BLOBs are private and one-time writes are bounded and ownership-scoped. |
| #54 Development public media reads and storage policy evidence | Anonymous media-read boundary | Route tests proving every request re-checks public visibility, non-public transitions fail closed, the kill switch blocks bytes, and no private storage identifier leaks. |
...
| #11 Publication state machine and public video asset creation | Public/private transition | State-machine and signed publish authorization tests proving only approval creates the published state and retained owned BLOB reference. |
