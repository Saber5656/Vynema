...
- Single job `checks` on `ubuntu-latest` (GitHub-hosted ONLY): checkout with credentials persistence disabled → Node 22 with implicit package-manager caching disabled → download Corepack v0.34.7 from the official `nodejs/corepack` GitHub asset and verify SHA-256 `7f7f581a5fae7946dce393c3ebd957e7449e73bb9b5bb219c566fa83d23412f4` before installation with package scripts disabled → Corepack activation of the exact `package.json#packageManager` value → Corepack/declaration/runtime version assertions → `pnpm install --frozen-lockfile` → shared-package build → `pnpm lint` → `pnpm typecheck` → `pnpm test` → `pnpm build`. Never disable Corepack package-manager signature verification. Do not configure pnpm caching before pnpm is active. Add `pnpm test:coverage` artifact upload once #20 lands.
...
- **Action pinning rule (normative)**: every action is pinned to a full 40-character commit SHA with a trailing version comment. Resolve SHAs at implementation time: `gh api repos/<owner>/<repo>/git/ref/tags/<vX.Y.Z> --jq .object.sha` (dereference annotated tags via `git/tags/<sha>` if type=tag). The current checks job uses only GitHub-owned `actions/checkout` and `actions/setup-node`; repository allowed-actions policy forbids `pnpm/action-setup`. The Node-bundled Corepack in the pinned Node 22.13.1 runtime has stale package-manager signing keys, so the job installs Corepack v0.34.7 from a fixed official GitHub release URL only after the same hard-coded SHA-256 asset check above, keeps signature verification enabled, and checks Corepack plus the exact repository package-manager declaration/runtime version. `actions/upload-artifact` may be introduced with a reviewed full-SHA pin only when #20 adds the corresponding artifact-producing check. Dependabot (already configured) keeps action pins updated.
