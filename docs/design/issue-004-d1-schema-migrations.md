...
- The repository runner owns an internal `schema_migrations` ledger with the
  applied version, filename, SHA-256, and epoch-ms application time. Status and
  apply operations fail closed if its contiguous rows differ from
  `PRAGMA user_version` or any applied repository file; never edit or rename an
  applied migration.
...
| restore preflight | incompatible migration metadata, orphaned foreign keys, non-pristine version-zero databases, migrated databases missing canonical tables/indexes/triggers, existing videos whose intent ownership or media references no longer satisfy the canonical trigger predicates, published/taken-down videos without retained approval evidence, and stale portable/FTS5 search-index content are rejected without changing the active database; a pristine version-zero source is accepted; a live WAL source is restored from a transactionally consistent snapshot without losing committed rows |
