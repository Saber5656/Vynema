...
| restore preflight | incompatible migration metadata, orphaned foreign keys, and non-pristine version-zero databases are rejected without changing the active database; a pristine version-zero source is accepted; a live WAL source is restored from a transactionally consistent snapshot without losing committed rows |
