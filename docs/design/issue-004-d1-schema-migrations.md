...
| restore preflight | incompatible migration metadata, orphaned foreign keys, non-pristine version-zero databases, migrated databases missing canonical tables/indexes/triggers, published/taken-down videos without retained approval evidence, and stale portable/FTS5 search-index content are rejected without changing the active database; a pristine version-zero source is accepted; a live WAL source is restored from a transactionally consistent snapshot without losing committed rows |
